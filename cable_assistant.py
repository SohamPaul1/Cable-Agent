...
try:
    ...
    print("❌ Missing required dependency 'mcp'. Install with: pip install mcp")
    sys.exit(1)
    ...
    async def _setup_session(self):
        ...
        if self.mcp_session:
            try:
                ...
                mcp_result = await self.mcp_session.list_tools()
                ...
                for tool in mcp_result.tools:
                    ...
                    azure_tools.append(
                        FunctionTool(
                            ...
                            # MCP 'inputSchema' maps directly to Azure 'parameters'
        ...
        # Add Local 'end_call' Tool
        logger.info("  - Adding local tool: end_call")
        azure_tools.append(
            FunctionTool(
                name="end_call",
                description="Call this tool to end the conversation immediately when the user says goodbye or indicates they are done.",
        ...
        if (
            ...
            or self.voice.startswith("bn-In-")
            or "-" in self.voice
        ):
            # Azure voice
        ...
        # voice_config.pitch = 0
        # voice_config.rate = 1.2
        # voice_config.style = "chat"

        turn_detection_config = AzureSemanticVad(
            threshold=0.5, prefix_padding_ms=300, silence_duration_ms=2000
        )
    ...
    async def _process_events(self):
        ...
        try:
            ...
            async for event in conn:
                ...
                if self.should_exit:
                    raise KeyboardInterrupt("Call ended by Anwesha")
    ...
    async def _handle_event(self, event):
        ...
        # --- TOOL EXECUTION LOGIC ---
            ...
            if hasattr(item, "type") and item.type == "function_call":
                logger.info(f"🤖 Azure requested tool: {item.name}")
                ...
                if item.name == "end_call":
                    ...
                    try:
                        await self.connection.send(
                            ClientEventConversationItemCreate(item=output_item)
                        )
                        await self.connection.send(ClientEventResponseCreate())
                    except Exception:
                        pass
                ...
                tool_output = ""
                try:
                    # 1. Parse arguments from Azure
                    args = json.loads(item.arguments) if item.arguments else {}

                    # 2. Execute on MCP Server
                    if self.mcp_session:
                        result = await self.mcp_session.call_tool(
                            item.name, arguments=args
                        )
                ...
                # 4. Send Output back to Azure
                output_item = FunctionCallOutputItem(
                    call_id=item.call_id, output=tool_output
                )
                await self.connection.send(
                    ClientEventConversationItemCreate(item=output_item)
                )
...
def parse_arguments():
    ...
    # Added MCP URL Argument with your specific default
    parser.add_argument(
        "--mcp-url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("VOICELIVE_VOICE", "bn-IN-TanishaaNeural"),
        choices=[
            "alloy",
            "echo",
            "fable",
...
async def main():
    ...
    try:
        ...
        if args.use_token_credential:
            ...
            )  # or DefaultAzureCredential() if needed
